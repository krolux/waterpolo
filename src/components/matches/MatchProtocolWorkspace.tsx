...
export function MatchProtocolWorkspace({ match, user, onClose, onProtocolChanged }: { match: Match; user: User; onClose: () => void; onProtocolChanged?: () => Promise<void> | void }) {
  ...
  const startMatch = () => {
    if (homePlayers.filter(player => player.isGoalkeeper).length > 2 || awayPlayers.filter(player => player.isGoalkeeper).length > 2) return alert("Każda drużyna może mieć maksymalnie dwóch bramkarzy.");
