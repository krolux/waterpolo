...
const uniqueCapNumbers = (players: ProtocolPlayer[]) => {
  const used = new Set<number>();
  return players.map(player => {
    let capNumber = Math.max(1, Number(player.capNumber) || player.slot);
    if (used.has(capNumber)) { capNumber = 1; while (used.has(capNumber)) capNumber += 1; }
    used.add(capNumber);
    return capNumber === player.capNumber ? player : { ...player, capNumber };
  });
};
...
export function MatchProtocolWorkspace({ match, user, onClose }: { match: Match; user: User; onClose: () => void }) {
  const [protocol, setProtocol] = React.useState<MatchProtocolDraft>(() => { const saved = loadProtocol(match.id); return { ...saved, homePlayers: uniqueCapNumbers(saved.homePlayers), awayPlayers: uniqueCapNumbers(saved.awayPlayers) }; });
  ...
  const selectedShootoutPlayers = activeShootoutTeam === "home" ? homePlayers : awayPlayers;
  ...
  const reopenProtocol = () => { if (!canApprove) return; setProtocol(p => ({ ...p, status: "setup", finishedAt: "", closedAt: undefined, closedBy: undefined, approvedAt: undefined, approvedBy: undefined })); };
  ...
  const updatePlayer = (side: ProtocolTeam, slot: number, field: "name" | "capNumber", value: string) => setProtocol(p => {
    ...
    const nextCapNumber = Math.max(1, Number(value) || slot);
    if (field === "capNumber" && p[key].some(item => item.slot !== slot && item.capNumber === nextCapNumber)) {
      alert(`Numer czepka ${nextCapNumber} jest już przypisany innemu zawodnikowi tej drużyny.`);
      return p;
    }
    const updated = { ...player, [field]: field === "capNumber" ? nextCapNumber : value };
