...
export function MatchProtocolWorkspace({ match, user, onClose, onProtocolChanged }: { match: Match; user: User; onClose: () => void; onProtocolChanged?: () => Promise<void> | void }) {
  ...
  const startMatch = () => {
    const validateTeam = (label: string, roster: ProtocolPlayer[]) => {
      ...
      const requiredGoalkeepers = active.length === 15 ? 2 : 1;
      if (goalkeepers !== requiredGoalkeepers) return `Drużyna ${label} przy ${active.length} zawodnikach musi mieć dokładnie ${requiredGoalkeepers} ${requiredGoalkeepers === 1 ? "bramkarza" : "bramkarzy"}.`;
      return null;
    };
