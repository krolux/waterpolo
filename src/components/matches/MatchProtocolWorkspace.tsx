...
export function MatchProtocolWorkspace({ match, user, onClose, onProtocolChanged, localOnly = false, initialProtocol, onLocalProtocolChange, readOnly = false }: { match: Match; user: User; onClose: () => void; onProtocolChanged?: (status: MatchProtocolDraft["status"]) => Promise<void> | void; localOnly?: boolean; initialProtocol?: MatchProtocolDraft; onLocalProtocolChange?: (protocol: MatchProtocolDraft) => void; readOnly?: boolean }) {
  ...
  }, [protocol, syncReady, syncProtocol, localOnly, onLocalProtocolChange, readOnly]);
  React.useEffect(() => {
    if (localOnly) return;
