...
import { approveRemoteMatchProtocol, listRemoteProtocolStatuses, type MatchProtocolDraft } from "../../lib/matchProtocol";
...
export function CompetitionsPageV2({ initialCode, isAdmin, clubs, refereeNames, delegateNames, onMatchesChanged, state, setState, effectiveUser, onPenaltiesChange }: Props) {
  ...
  const [protocolStatusByMatchId, setProtocolStatusByMatchId] = React.useState<Record<string, MatchProtocolDraft["status"]>>({});
  const [approvingProtocolId, setApprovingProtocolId] = React.useState<string | null>(null);
  const [reviewingMatchId, setReviewingMatchId] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    let cancelled = false;
    if (!effectiveUser || !context.matches.length) { setProtocolStatusByMatchId({}); return; }
    listRemoteProtocolStatuses(context.matches.map(match => match.id))
      .then(statuses => { if (!cancelled) setProtocolStatusByMatchId(statuses); })
      .catch(() => { if (!cancelled) setProtocolStatusByMatchId({}); });
    return () => { cancelled = true; };
  ...
  const canApproveProtocol = React.useCallback((match: { delegate?: string | null }) => !!effectiveUser && (isAdmin || (!!match.delegate && effectiveUser.name === match.delegate)), [effectiveUser, isAdmin]);
  ...
  const approveProtocolFromMatch = async (matchId: string) => {
    ...
    try {
      const approved = await approveRemoteMatchProtocol(match, effectiveUser.name);
      const score = approved.events.reduce((value, event) => event.kind === "goal" || event.kind === "shootout_goal" ? { ...value, [event.team]: value[event.team] + 1 } : value, { home: 0, away: 0 });
      setState(old => ({ ...old, matches: old.matches.map(item => item.id === matchId ? { ...item, result: `${score.home}:${score.away}`, shootout: approved.events.some(event => event.period === "PS") } : item) }));
      setProtocolStatusByMatchId(current => ({ ...current, [matchId]: "approved" }));
      onPenaltiesChange();
      await onMatchesChanged();
      await reload();
    } catch (error) {
      alert("Nie udało się zatwierdzić protokołu: " + (error instanceof Error ? error.message : String(error)));
    } finally { setApprovingProtocolId(null); }
  };
        ...
        return <article key={m.id} className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm">
          <div className="p-3">
            ...
            {(isUserReferee || isAdmin || (effectiveUser && actionMatch)) && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-sky-50 pt-3">
              ...
              {privateProtocolEnabled && actionMatch && canOpenProtocol(m) && <button onClick={() => setProtocolMatchId(m.id)} className="inline-flex items-center gap-1 rounded-lg border border-amber-300 bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-800"><FileText className="h-3.5 w-3.5" />Protokół</button>}
              {protocolStatusByMatchId[m.id] === "submitted" && canApproveProtocol(m) ? <button disabled={approvingProtocolId === m.id} onClick={() => setReviewingMatchId(m.id)} className="rounded-lg bg-green-600 px-2.5 py-1.5 text-xs font-semibold text-white disabled:opacity-50">{approvingProtocolId === m.id ? "Zatwierdzanie…" : "Zatwierdź protokół"}</button> : null}
              {protocolStatusByMatchId[m.id] === "approved" ? <span className="rounded-full bg-green-50 px-2 py-1 text-xs font-semibold text-green-700">Protokół zatwierdzony</span> : null}
            </div>}
          </div>
  ...
  return <div className="min-w-0 space-y-4">
    ...
    {privateProtocolEnabled && protocolMatch && effectiveUser ? <MatchProtocolWorkspace match={protocolMatch} user={effectiveUser} onClose={() => setProtocolMatchId(null)} onProtocolChanged={async status => { setProtocolStatusByMatchId(current => ({ ...current, [protocolMatch.id]: status })); if (status === "live" || status === "setup") setState(old => ({ ...old, matches: old.matches.map(item => item.id === protocolMatch.id ? { ...item, result: "", shootout: false } : item) })); await onMatchesChanged(); await reload(); onPenaltiesChange(); }} /> : null}
    {reviewingMatchId && (()=>{const match=state.matches.find(item=>item.id===reviewingMatchId);return match?<RefereeReviewModal matchId={match.id} referees={match.referees} onCancel={()=>setReviewingMatchId(null)} onSaved={async()=>{setReviewingMatchId(null);await approveProtocolFromMatch(match.id);}}/>:null;})()}
  </div>;
}
