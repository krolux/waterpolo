...
export function CompetitionsPageV2({ initialCode, isAdmin, clubs, refereeNames, delegateNames, onMatchesChanged, state, setState, effectiveUser, onPenaltiesChange }: Props) {
  ...
  return <div className="min-w-0 space-y-4">
    ...
    {privateProtocolEnabled && protocolMatch && effectiveUser ? <MatchProtocolWorkspace match={protocolMatch} user={effectiveUser} onClose={() => setProtocolMatchId(null)} onProtocolChanged={async () => { setProtocolStatusByMatchId(current => ({ ...current, [protocolMatch.id]: "live" })); setState(old => ({ ...old, matches: old.matches.map(item => item.id === protocolMatch.id ? { ...item, result: "", shootout: false } : item) })); await onMatchesChanged(); await reload(); onPenaltiesChange(); }} /> : null}
  </div>;
}
