...
export function CompetitionsPageV2({ initialCode, isAdmin, clubs, refereeNames, delegateNames, onMatchesChanged, state, setState, effectiveUser, onPenaltiesChange }: Props) {
  ...
  const approveProtocolFromMatch = async (matchId: string) => {
    ...
    if (!confirm("Zatwierdzić protokół? Wynik i zawieszenia zostaną zapisane, a gospodarz nie będzie mógł go dalej edytować.")) return;
        ...
        return <article key={m.id} className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm">
          <div className="p-3">
            ...
            {(isUserReferee || isAdmin || (effectiveUser && actionMatch)) && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-sky-50 pt-3">
              ...
              {protocolStatusByMatchId[m.id] === "submitted" && canApproveProtocol(m) ? <button disabled={approvingProtocolId === m.id} onClick={() => void approveProtocolFromMatch(m.id)} className="rounded-lg bg-green-600 px-2.5 py-1.5 text-xs font-semibold text-white disabled:opacity-50">{approvingProtocolId === m.id ? "Zatwierdzanie…" : "Zatwierdź protokół"}</button> : null}
