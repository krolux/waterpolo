...
import { CalendarDays, Check, Edit, MapPin, Plus, RefreshCw, Trash2, UserRoundCheck, X } from "lucide-react";
    ...
    <div className="space-y-3 p-3 md:hidden">
      {items.map(m => {
        ...
        return <article key={m.id} className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm">
          <div className="p-3">
            ...
            {(isUserReferee || isAdmin || (effectiveUser && actionMatch)) && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-sky-50 pt-3">
              ...
              {isAdmin && <>
                ...
                <button aria-label={`Usuń ${m.home} – ${m.away}`} onClick={() => void removeMatch(m.id)} className="rounded-lg border p-1.5 text-red-600"><Trash2 className="h-4 w-4" /></button>
              </>}
          ...
          {effectiveUser && actionMatch && <div className="px-3 pb-3"><MatchDocuments match={actionMatch} /></div>}
        </article>;
      })}
    </div>
  );
